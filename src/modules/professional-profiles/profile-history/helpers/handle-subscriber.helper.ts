...
export function handleProfileHistoryEvent (profile_id: string, event_type: AuditEvent, action: string, event_description: string, table_name?: string, module?: AuditModule) {
    ...
    if (!profile_history) {
      ...
      if (currentUser) {
        RequestContext.setEvent({
          ...
          module: module || AuditModule.Professional,
          ip: currentUser.ip,
        })
      }
    }
